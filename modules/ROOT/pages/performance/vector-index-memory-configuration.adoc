...
For comparison adding a Node or Integer property requires only a few bytes.
Vectors can quickly become the largest consumer of memory and disk in a database.
To calculate the memory requirements we'll first consider the size of our vectors to estimate the total amount of vector data. 
Then we'll consider the index's configuration and requirements.

=== Vector sizes

An important first step is to esimate the number of bytes of vector data the database will store.
The bytes required by a single vector can be calculated using this formula or using the table below:
...
By default, Neo4j vector indexes use quantization to reduce the memory required to store the vectors.
Quantization reduces the size of the vector by a factor of 4 and improves performance, at a small cost to recall accuracy.
Recall accuracy can be increased by increasing HNSW parameters such as EF Search, or disabling quantization, see https://neo4j.com/docs/cypher-manual/current/indexes/semantic-indexes/vector-indexes/#config-vector.quantization.enabled.
